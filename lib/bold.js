...
async function createLink({ amountCOP, description, reference, callbackUrl }) {
  ...
  // TEMP (sandbox only): reveal Bold's real response shape so the field mapping
  // below is confirmed against reality, then this comes out.
  if (env() === "test") console.log("[bold createLink] status", res.status, "body", text.slice(0, 600));
