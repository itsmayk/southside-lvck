...
// TEMP (sandbox): stash the last webhook Bold sends so its real shape can be
// read back regardless of log timing. Removed once verification is locked.
async function recordDelivery(obj) {
  const r = redis();
  if (!r) return;
  await r.set("debug:last-webhook", JSON.stringify(obj), { ex: 7200 });
}
async function lastDelivery() {
  const r = redis();
  if (!r) return null;
  return r.get("debug:last-webhook");
}

module.exports = {
  isReady, availability, reserve, referenceSize, confirm, release,
  recordDelivery, lastDelivery,
};
